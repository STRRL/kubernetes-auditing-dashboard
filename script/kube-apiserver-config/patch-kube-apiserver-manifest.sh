...
yq -i '.spec.containers[0].command |= . + ["--audit-policy-file=/kube-apiserver-config/audit-policy.yaml", "--audit-webhook-config-file=/kube-apiserver-config/audit-webhook.kubeconfig", "--audit-webhook-initial-backoff=1s"]' /etc/kubernetes/manifests/kube-apiserver.yaml
